...
namespace DevTrack.Infrastructure.Services
{
    public class InvitationService : IInvitationService
    {
        ...
        public async Task<IList<string>> GetUserEmails(UserEmailListBO model)
        {
            var projectUsers = _applicationUnitOfWork.Projects.GetUserEmailsByProjectID(model.ProjectId).ToList();
            var pendingEmails = (await GetPendingInvitationEmailsByProjectId(model.ProjectId)).ToList();
            projectUsers.AddRange(pendingEmails);
            
            return projectUsers;
        }

        public async Task<IList<string>> GetPendingInvitationEmailsByProjectId(Guid projectId)
        {
            var pendingEmails = _applicationUnitOfWork.Invitations.GetPendingInvitationEmailsByProjectId(projectId);
